...
check_http() {
  ...
  curl -sSL -c "$cookie_jar" -b "$cookie_jar" -D "$headers" -o /dev/null "$url"
  local status
...
curl -sS "$API_BASE_URL/store/products?limit=20&region_id=$REGION_ID" \
...
curl -sS -X POST "$API_BASE_URL/store/carts" \
...
curl -sS -X POST "$API_BASE_URL/store/carts/$cart_id/line-items" \
  -H "x-publishable-api-key: $PUBLISHABLE_KEY" \
  -H "Content-Type: application/json" \
